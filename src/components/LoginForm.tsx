...
export function LoginForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-stone-600">Use your Accolade Stays portal account.</p>
        </div>
      </div>
      ...
      <div className="grid gap-4">
        <label className="grid gap-2 text-sm font-medium text-brand-ink" htmlFor="email">
          ...
          <input
            ...
            className="min-h-12 rounded-md border border-stone-300 bg-white px-3 text-base text-brand-ink outline-none transition focus:border-brand-moss focus:ring-2 focus:ring-brand-moss/20"
            {...register("email")}
          />
        ...
        <button
          type="submit"
          disabled={isPending}
          className="mt-2 min-h-12 rounded-md bg-brand-moss px-4 text-base font-semibold text-white transition hover:bg-brand-ink focus:outline-none focus:ring-2 focus:ring-brand-moss focus:ring-offset-2 disabled:cursor-wait disabled:opacity-70"
        >
